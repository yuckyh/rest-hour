...
```bash
rest-hour$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rest-hour$ sam local start-api
rest-hour$ curl http://localhost:3000/
```
...
```bash
rest-hour$ sam logs -n HelloWorldFunction --stack-name rest-hour --tail
```
